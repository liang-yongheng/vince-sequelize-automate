...
function getFieldName(fieldName, camelCase, tablePrefix = '') {
  if (tablePrefix) fieldName = fieldName.replace(tablePrefix, '');
...
function getModelDefinitions(tables, options) {
  const { camelCase, fileNameCamelCase, dialect, tablePrefix } = options || {};
